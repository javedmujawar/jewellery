...
const CountryList = () => {
  ...
  const getAllList = async () => {
    const b = new BaseApi();
    const result = await b.getAll("countries");
    setData(result);
  ...
  const handleOk = async () => {
    try {
      ...
      const res = await b.request("countries", postData, "patch");
      if (res.status === 200) {
        setModalVisible(false);
        setDeletedId(0);
        navigate("/country", {
  ...
  const handlePDF = () => {
    ...
    const headers = [["Name","Short Name","Phone Code", "Status"]];
    const tdata = data.map(elt=> [elt.name,elt.shortName,elt.phoneCode,elt.status]); 
