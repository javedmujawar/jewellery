...
const CustomerCategoriesList = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        return (
          <span>
            <Link to={"/customer-categories/edit/" + record.id}>
              <Button
                type="primary"
                id="btnEdit"
                name="btnEdit"
  ...
  const getAllList = async () => {
    const b = new BaseApi();
    const result = await b.getAll("customercategories");
    //  console.log(result);
    setData(result);
  };
  ...
  const handleOk = async () => {
    try {
      ...
      const res = await b.request("customercategories", postData, "patch");
      if (res.status === 200) {
        setModalVisible(false);
        setDeletedId(0);
       // getAllList();  
       navigate('/customer-categories', { state: { message:'Record is deleted successfully.' }}) 
    ...
    <Grid container spacing={3}>
      ...
      <Grid item xs={12}>
        <Stack
          ...
          sx={{ mb: { xs: -0.5, sm: 0.5 } }}
        >
          <Typography variant="h3">Customer Categories List</Typography>
          ...
          <Button
            ...
            name="btnCreate"
            onClick={() => {
              navigate("/customer-categories/add");
            }}
          >
            Create
          </Button>
